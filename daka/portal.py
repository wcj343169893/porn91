...
class Portal:
    username = ''
    password = ''
    baseUrl = "https://portal.gamehours.com"
    ...
    # 获取打卡记录
    def fetch_card_list(self):
        endpoint = "/ehrportal/DEPT/Personal_CardData_DataList.asp?104eHrTime=" + str(int(time.time() * 1000))
        # print(url)
        # 发送GET请求
        ...
        # 获取所有的打卡记录 <tr ID='tr62797505' height='24'
        ...
        # 打印打卡记录数组
        # _LOG.info(card_records)
        # 打印内容
        # print(response.text)
        # 只返还第一笔就行
        if len(card_records) > 0:
            last_card_date = card_records[0]
            if len(card_records) > 1 and card_records[0] < card_records[1]:
                last_card_date = card_records[1]
            _LOG.info("最近一次打卡记录：%s", last_card_date)
            return last_card_date
        else:
            _LOG.info("没有打卡记录，可能未登录")
            return ""
